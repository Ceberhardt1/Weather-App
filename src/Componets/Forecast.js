...
export default function Forecast() {
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          var response = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${pos.coords.latitude}&lon=${pos.coords.longitude}&units=metric&id=524901&appid=ca8c2c7970a09dc296d9b3cfc4d06940`)
          var data = await response.json();
          console.log(data);
          console.log(pos);
          if (data.weather[0].main === "Rain" || data.weather[0].main === "Thunderstorm" || data.weather[0].main === "Drizzle" || data.weather[0].main === "Mist") {
            setIcon("RAIN");
          }
          else if (data.weather[0].main === "Snow") {
            setIcon("SNOW")
          }
          else if (data.weather[0].main === "Fog" || data.weather[0].main === "Smoke" || data.weather[0].main === "Haze" || data.weather[0].main === "Dust") {
            setIcon("FOG")
          }
  ...
  return (
    <div className='overall-container'>
      ...
      {tabName === "weather" ?
        <>
          <Weather city={city} setCity={setCity} icon={icon} setIcon={setIcon} />
        </> :
        <>
        ...
        <div className='left-container'>
          ...
          <div className='bottom-left'>
            <div className='clock-left'>
              <h1><Clock format={'HH:mm:ss'} ticking={true} timezone={'US/Eastern'} /></h1>
...
function ForecastSection({ city }) {
  ...
  useEffect(() => {
    ...
    async function getForecast(lat, lon) {
      var response = await fetch(`https://api.openweathermap.org/data/2.5/onecall?units=imperial&lat=${lat}&lon=${lon}&exclude=current,hourly,minutely&appid=ca8c2c7970a09dc296d9b3cfc4d06940`)
      var data = await response.json()
      console.log(data)
        ...
        {forecast.map((day) => {
          return (
            <div>
              ...
              <p><img src={`https://openweathermap.org/img/wn/${day.weather?.[0].icon}@2x.png`} alt='icons' width={70} /></p>
              <h2>{day.weather?.[0].description}</h2>
              <h3>{Math.round(day.temp.day)}°F/{Math.round(day.temp.night)}°F</h3>
            </div>
          )
        })}
      </div>
